...
N_VOLS = 1000         # total vols — used for Zernike analysis (figs 10, 11)
N_NRMSE_VOLS = 100    # first N vols also get resolution NRMSE sweep (fig 17)
...
MEAS_TYPES = ['OPL', 'OPD_TT']   # projection / sinogram type
EVAL_TYPES = ['OPL', 'OPD_TT']   # evaluation type (TTP included vs removed from recon)
...
roi_zern = roi_per_res[ZERN_RESOLUTION]
...
# nrmse_arr[vol, meas, eval, geo, res]  — first N_NRMSE_VOLS volumes only
nrmse_arr = np.zeros((N_NRMSE_VOLS, n_meas, n_eval, n_geos, n_res))
# zernike_mse_arr[vol, meas, geo, zmode]  — all N_VOLS, OPD_TT eval at ZERN_RESOLUTION
zernike_mse_arr = np.zeros((N_VOLS, n_meas, n_geos, n_zmodes))
...
for vol_idx in trange(N_VOLS, desc='Volumes'):
    ...
    for geo_idx, (geo_label, half_ext, n_views) in enumerate(tqdm(GEOMETRIES, desc='Geometries', leave=False)):
        ...
        for meas_idx, proj_type in enumerate(tqdm(MEAS_TYPES, desc='Meas', leave=False)):
            ...
            # ---- Zernike at ZERN_RESOLUTION (all vols, OPD_TT eval) ----
            ...
            zernike_mse_arr[vol_idx, meas_idx, geo_idx, :N_OSA_MODES] = per_mode_mse
            zernike_mse_arr[vol_idx, meas_idx, geo_idx, -1] = float(np.mean(error_z[roi_zern] ** 2))

            # ---- NRMSE resolution sweep (first N_NRMSE_VOLS only) ----
            if vol_idx < N_NRMSE_VOLS:
                for res_idx, s in enumerate(RESOLUTIONS):
                    ...
                    # OPL eval (eval_idx=0): compare against OPL GT (no TTP removal)
                    nrmse_arr[vol_idx, meas_idx, 0, geo_idx, res_idx] = float(
                        va.nrmse_over_roi(jnp.array(gt_s_raw), jnp.array(recon_s_raw), roi_s_jnp, option=2)
                    )
                    # OPD_TT eval (eval_idx=1): compare against TTP-removed GT
                    nrmse_arr[vol_idx, meas_idx, 1, geo_idx, res_idx] = float(
                        va.nrmse_over_roi(jnp.array(gt_s_ttp), jnp.array(recon_s_ttp), roi_s_jnp, option=2)
                    )

    if (vol_idx + 1) % 50 == 0:
        np.savez(
            DATA_DIR / 'fig10_11_17_3v2_11v16_partial.npz',
            nrmse=nrmse_arr,
...
# ============================================================
np.savez(
    DATA_DIR / 'fig10_11_17_3v2_11v16.npz',
    nrmse=nrmse_arr,
    zernike_mse=zernike_mse_arr,
    resolutions=np.array(RESOLUTIONS),
    geometry_names=np.array([g[0] for g in GEOMETRIES]),
...
print('Done. Saved data/fig10_11_17_3v2_11v16.npz')
