...
warn_and_confirm('fig13_14_15_17')
...
N_VOLS = 1000        # total reconstructions — used for Zernike (figs 14/15)
N_NRMSE_VOLS = 100  # first N vols also get resolution NRMSE (figs 13/17)
...
HALF_EXTENT_DEG = 4.0
N_VIEWS = 7
# Measurement types (sinogram input)
MEAS_TYPES = ['OPL', 'OPD_TT']
# Evaluation types (how GT and recon are compared for NRMSE):
...
for vol_idx in trange(N_VOLS, desc='Volumes'):
    ...
    for meas_idx, proj_type in enumerate(tqdm(MEAS_TYPES, desc='Meas', leave=False)):
        ...
        # ---- Zernike at ZERN_RESOLUTION (all vols, figs 14/15) ----
        # Raw sections — no TTP removal — all Zernike modes including tip/tilt/piston
        ...
        # ---- NRMSE at all resolutions (first N_NRMSE_VOLS vols only, figs 13/17) ----
